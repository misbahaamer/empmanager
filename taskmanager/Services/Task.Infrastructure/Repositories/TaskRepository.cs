...
namespace Task.Infrastructure.Repositories
{
    public class TaskRepository : RepositoryBase<Employee>, IEmployeeRepository
    {
        ...
        public async Task<IEnumerable<Employee>> GetTasksByDueDate(DateTime date)
        {
        ...
        public async Task<IEnumerable<Employee>> GetTasksByName(string name)
        {
            var taskList = await _dbContext.Tasks
                                .Where(o => o.Name == name)
                                .ToListAsync();
        ...
        public async Task<IEnumerable<Employee>> GetTasksByPriorityType(string priority)
        {
            var taskList = await _dbContext.Tasks
        ...
        public async Task<IEnumerable<Employee>> GetTasksByStatus(string status)
        {
            var taskList = await _dbContext.Tasks
